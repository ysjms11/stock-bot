...
MCP_TOOLS = [
    ...
    {"name": "read_report_pdf",
     "description": "리포트 PDF 읽기. mode=image(기본, 페이지별 PNG 이미지·최대 20p·claude.ai 100장 제한 적용), text(텍스트 전페이지 추출·페이지/이미지 제한 없음·차트는 글자만), pdf(PDF 원본 임베드·이미지 0장·100장 제한 무관·실험적). 종목코드만으로 최신 리포트 자동 조회.",
     "inputSchema": {"type": "object",
                     "properties": {
                         "ticker":    {"type": "string",  "description": "종목코드"},
                         "report_id": {"type": "integer", "description": "리포트 ID (manage_report에서 확인, 생략 시 해당 종목 최신 PDF)"},
                         "pages":     {"type": "string",  "description": "페이지 범위 (예: '1-3', 생략 시 전체). pdf 모드에서는 무시됨"},
