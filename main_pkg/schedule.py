...
def register_all_schedules(jq):
    ...
    jq.run_daily(daily_nps_dart_increment, time=dtime(4, 2, tzinfo=KST), name="nps_dart_inc")  # +2m (stagger: weekly_us_analyst_sync 04:00)
    ...
    jq.run_daily(sunday_30_reminder,      time=dtime(19, 2, tzinfo=KST), days=(0,), name="sunday_30")  # +2m (stagger: weekly_us_analyst 19:00)
    ...
    jq.run_daily(daily_event_d1_alert, time=dtime(19, 31, tzinfo=KST), days=(0, 1, 2, 3, 4, 5, 6), name="event_d1")  # +1m (stagger: daily_consensus 19:30)
    jq.run_daily(daily_pension_collect, time=dtime(16, 32, tzinfo=KST), days=(1, 2, 3, 4, 5), name="pension_collect")  # +2m (rate limit stagger)
    jq.run_daily(daily_pension_alert,   time=dtime(19,  1, tzinfo=KST), days=(1, 2, 3, 4, 5), name="pension_alert")  # +1m (stagger: watch_change 19:00)
    jq.run_daily(weekly_report_digest_notify, time=dtime(19, 7, tzinfo=KST), days=(0,), name="weekly_report_digest")
    jq.run_daily(weekly_sanity_check,     time=dtime(7,  6, tzinfo=KST), days=(0,), name="weekly_sanity")  # +1m (stagger: weekly_consensus 07:05)
