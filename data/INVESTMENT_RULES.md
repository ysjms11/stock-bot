...
>
> **⭐ 문서 우선순위**: 매도·현금·단일캡 규칙은 **본 문서(§3·§5) + 6/1 EXIT판(KR/US_EXIT)이 단일 기준**이다. DEEPSEARCH 등 구판(v4)에 남은 충돌 표현 — "부분 매도/정리 30~50%", "단일 종목 35%", "경로3 보수 임계(격차 5%p/12개월)" — 은 **모두 폐기·대체됨**(전량 이진 / 단일 25% 하드캡 / 능동 경로3). 구판 텍스트는 점진 정리 중이나, **충돌 시 본 문서가 우선**한다.
...
- **단일 종목 25% 하드캡** (절대 한도, 시총 증가 포함) → 초과 시 축소. **클러스터(섹터) 35% 소프트캡** — 단 의도적 매크로 베팅(예: 조선+전력 인프라 슈퍼사이클)은 사전 기록 시 예외(우발적 집중만 차단). (구판 '단일 35%' 표기는 본 **25% 하드캡**으로 대체 — 2026-06-04. 사용자 규칙·HD조선 ~24% 모니터링과 정합.)
...
| 단일 종목 하드캡 | 35% (구판) | **단일 25% 하드 + 클러스터(섹터) 35% 소프트**(의도적 매크로 베팅 예외) — 사용자 규칙·HD조선 ~24% 모니터링 정합 |
| 문서 우선순위 | 암묵 | **명문화**: 마스터(§3·§5)+6/1 EXIT 단일 기준, 구판 충돌 표현(부분매도·단일35%·경로3 보수임계) 무효화 |
...
| 섹터 상한 50% | 사용 | 폐지 (집중 투자 원칙, 킬스위치 대체) — ※ 2026-06-04 클러스터 35% 소프트캡으로 부분 복원 |
