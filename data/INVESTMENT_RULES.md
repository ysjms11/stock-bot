...
## 관세청 수출 데이터 (2026-04-23, 3개월 관찰 모드)

### 역할
- **매수 시그널 아님 — 업종/종목 확인 지표**
- 실증(2026-04-23 딥서치): 반도체는 **동행**, 변압기/조선/방산은 **후행**, 라면/화장품 상관 약함
- 유튜버/블로거 주장 대부분 허위 (라면 r=0.98 주장 → 실측 +0.41)
- **3개월 관찰 모드** 동안 의사결정 가중치 = 0, 기록만 (`data/research/export_validation_log.md`)

### 업종 신뢰도 (실증 기반)

| 등급 | 업종 | 종목 | r | 해석 |
|------|------|------|---|------|
| 🟢 고신뢰 | 반도체 (DRAM/HBM) | 삼성전자, SK하이닉스 | +0.93~0.96 | **동행** (lag=0, 이미 반영) |
| 🟢 고신뢰 | MLCC/콘덴서 | 삼성전기 | 미검증 | 업계 통상 (수출비중 70%+) |
| 🟡 중신뢰 | 양극재 | 엘앤에프, 에코프로비엠 | +0.50~0.72 | lag=1~6개월 선행 |
| 🟡 중신뢰 | 2차전지 셀 | 삼성SDI | +0.54 | 동행 (해외법인 주의) |
| 🔴 저신뢰 | 변압기/조선/방산/원전 | HD현대일렉, HD조선, LIG넥스원 등 | < 0.35 | **수주공시 우선** — 수출은 후행 |
| 🔴 저신뢰 | 라면/화장품 | 삼양식품, 아모레 | 0.2~0.4 | HS 범용, 개별 종목 괴리 큼 |

### 사용 원칙 3가지

1. **동행 vs 후행 구분** — 반도체 수출 폭발 시점엔 주가 이미 상승. "수출 좋음 = 매수" 전략 X. DRAM 스팟가·HBM 가이던스가 진짜 선행.
2. **HS 세분화 필수** — HS 901890(미용기기), 330499(화장품), 3004(제약) 같은 범용 코드는 여러 종목 섞여 역상관 가능. 10자리 세분화 전엔 참고용.
3. **수주 기반 업종은 수출 시그널 믿지 마라** — 조선/방산/원전/변압기는 통관 = 납품 시점 = 수주 후 2~24개월 후행. 이런 업종은 **수주 공시**가 진짜 알파.

### MCP 도구

- `get_export_trend(ticker)` — 기본 summary
- `get_export_trend(ticker, mode="country_breakdown")` — 국가별 분해 (미중일/대만/홍콩 로테이션 확인)
- `get_export_trend(ticker, mode="timeseries")` — 12개월 YoY/MoM/QoQ + 피크아웃 3지표
- `get_export_trend(ticker, mode="asp")` — 판가($/kg) 역산 (수출금액 ÷ 중량)

### 자동 알림

- **매월 11일 09:00 KST** — 1~10일 잠정치 (korea.kr PDF)
- **매월 21일 09:00 KST** — 1~20일 잠정치
- **매월 16일 03:00 KST** — 전월 확정치 + 30종목 전체 YoY (data.go.kr API)

매핑: `data/hs_ticker_map.json` (30종목, 실증 재조정 v2). Phase 2 400종목 검증 후 확장 예정.

### 역상관 알파 (2026-04-23 추가, 5종목)

화학 범용품(HS 390110 폴리에틸렌) 수출 급증 = 공급과잉/스프레드 악화 = 주가 약세. 즉 **수출 데이터 ↑ → 매도 시그널**.

- **대상**: LG화학(051910), LG화학우(051915), 롯데케미칼(011170), 대한유화(006650), 금호석유화학(011780)
- **플래그**: `hs_ticker_map.json` 에 `inverse_alpha: true`
- **해석**: YoY +20% 급증 시 주의. 알림 메시지 반대 방향
- **근거**: Phase 2 실증 (lag 6m, r=-0.61~-0.72), B 에이전트 분류 A 카테고리

### 3개월 후 재평가 기준 (2026-07-23)

- **유지**: 고신뢰 종목 방향 일치율 > 55% & 초과수익 양수
- **개선**: 45~55% (특정 업종만 유효)
- **롤백**: < 45% — 4/19 거버넌스 같은 "간판만 비슷" 증상
