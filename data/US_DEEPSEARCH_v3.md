...
> 학술·실무 검증 반영 최종본 (2026.04.17: K/G 국면 STEP 0 추가)
> [티커], [현재가], [사이즈] 등 대괄호만 바꿔서 복붙 (사이즈 = Core / Standard / Starter)
...
봇: get_regime
웹서치: 최근 1주 S&P 500 주도 요인 (Fed·CPI·어닝 시즌 중 어느 쪽?)
□ 🟢 / 🟡 / 🔴 레짐 확인
□ 최근 시장 변동 원인 분류 (Gordon: P=D(1+g)/(K-g)):
  - K(금리·유가·달러·유동성) 주도? → PE 수축/확장 구간, thesis intact 시 진입 기회
  - G(실적·가이던스 하향 연쇄) 주도? → 섹터 이익 하향 리스크 — 이 종목 컨센 EPS 궤적 재확인 필수
  - 혼합 → 1차 트랜치만, 2차는 다음 CPI/어닝 후
※ 🔴에도 매수 가능 (VIX30+ 12M 양수 확률 75%). 단 G 쇼크면 Core 후보도 thesis 재검증 후.
...
□ Short Float 10%+ 경계 / 20%+ 고위험 / 30%+ 극단 (INVESTMENT_RULES v6)  □ UOA(비정상 옵션) 콜/풋 편향  □ 13D
...
━━ STEP 7. 킬질문 10개 + 3-Gate 판정 (8분) ★9→10개 ━━
머스트패스(실패 시 자동 탈락):
...
**3-Gate 판정** (INVESTMENT_RULES v6 §1):
  1) 산업 흐름 (Step 2 ①/③ 구조적 수요·컨센 상향): [통과/미달]
  2) 리더십 + 재무 (Step 2 ② 해자 + Step 3 Altman>2.6/Beneish<-1.78/FCF): [통과/미달]
  3) 실적 모멘텀 + 킬질문 머스트 3통과: [통과/미달]
  → 3개 통과: Core/Standard 후보 (RR 1:1.5+)
  → 2개 통과: Starter (RR 1:2 필수)
  → 1개 이하: 매수 금지
...
레짐+K/G 분류 → 3대 질문 → PEG 참고(2.0 경계, v6 §1 Step 4) → 3-Gate 결과 + 사이즈 → 감시가 → set_alert.
```
...
5개 질문 (INVESTMENT_RULES v6 §4): 1.처음 매수 이유 유효? 2.3-Gate 여전히 통과? 3.펀더멘털 추이? 4.리더십 유지? 5.리스크 지표(Altman/Beneish/Short) 악화?
```
...
감시가 10% 이내만: 웹서치 "[티커] earnings guidance" → 3대 질문 속판 → 3-Gate 재판정/감시가 재조정.
...
■ 3-Gate 판정: [ 3/3 Core · Standard / 2/3 Starter / ≤1 매수금지 ] (INVESTMENT_RULES v6 §1)

■ 시장 국면 (STEP 0):
  레짐: 🟢/🟡/🔴 | K/G 분류: K주도 / G주도 / 혼합
  → 진입 전략: [현재가 1차 / 감시가 대기 / 혼합 1차만]
