...
| 2026-04-19 | 거버넌스/밸류업 1단계 (자사주 감지) | `filter_governance_disclosures` + `daily_governance_alert` 매일 19:10 평일. 키워드 14종(자기주식/자사주 × 취득/소각/처분/매입). 전종목 DART 공시 스캔 → rcept_no 기반 30일 쿨다운. 유형별 그룹(소각>취득>처분>기타), 타입당 10건 상한. 워치/포트 👀 플래그. 스모크: 7일치 2건(더블유게임즈/크레버스 처분). 2단계 배당 변화 감지는 후속. |
