...
0. **맥미니 git pull + 재시작** — 이번 세션 큰 변경 배포 반영:
   ...
   - **관세청 수출 모듈 완전 롤백 (4/23)** — `trade_api.py` + MCP `get_export_trend` + 월 3회 잡 + DB 테이블 전부 제거

1. **다음 일요일 03:00 KST 주간 US harvest 첫 실행 확인** — 1,010종목 × ~33분 스캔. 애널 풀 확장 트래킹.
