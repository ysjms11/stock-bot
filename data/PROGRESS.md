...
| 2026-04-23 | HS 매핑 실증 기반 재조정 v2 (46d1662) | 병렬 딥서치로 기존 유튜버 주장 대부분 허위 확인. 라면 r=0.98 → 실측 +0.41, 미용의료기기 r=0.97 → -0.57. 반도체 동행 (lag=0, r=+0.93~0.96). 고신뢰 8→3개 (삼성전자/SK하이닉스/삼성전기). |
| 2026-04-23 | 400종목 실증 대규모 검증 | 시총 TOP 400 → 섹터 기반 HS 매핑 258종목 → yfinance 250 → 🟢 42종목 + 🟡 47종목 + ⚠️ 역상관 62종목. 반도체/자동차/건설기계 100% 통과. **화학 HS 390110 역상관 12종목 = 스프레드 프록시 발견** (LG화학/롯데케미칼/금호석유 등). K-POP 852349 가설 부정 (혼합 노이즈). 자동차 lag=6 실증. 매핑 확장 89종목 풀 확보. |
...
| 2026-04-23 | 룰 문서 수출 섹션 추가 (edb5dbb) | INVESTMENT_RULES §5 관세청 수출 데이터 신설 + KR_DEEPSEARCH Step 2 `get_export_trend` 통합. 3개월 관찰 모드 (55%/45% 일치율 기준). |
...
| 2026-04-23 | HS 매핑 Phase 2 확장 v3 (cb8c279) | 400종목 실증 결과 적용. 30 → 108종목 (+78). 고신뢰 3→36. Agent 2 예측 실증 통과(산일전기/리노공업/이수페타시스), K-POP 음반 가설 부정. |
| 2026-04-23 | 역상관 알파 5종목 추가 v3.1 | B 에이전트 역상관 62종목 분류: A(알파)=8 / B(HS오매핑)=31 / C(노이즈)=14 / D=6. HS 390110(PE) 공급과잉 프록시 확인. LG화학/롯데케미칼/대한유화/금호석유/LG화학우 (inverse_alpha=True). 역상관 50%는 섹터→HS 자동 매핑 결함 — Phase 3에서 DART 사업보고서 파싱 필요. 지주사 부호 반전(아모레홀딩스 vs 본사) 구조적 증거. |
