...
0e. **US 애널 레이팅 오탐 수정 배포 (4/21, d1b2c1d)** — 근본원인: 첫 수집 시 수개월치 레이팅이 `fetched_at=now()`로 INSERT → `fetched_at >= now-24h` 필터 전부 통과. 수정: `rating_date >= date('now', '-4 days')` 기준으로 변경. Hold→Hold 무변화 제외. 알림에 report_date + 경과일수 표시. **맥미니 git pull + 재시작 필요.** 재가동 후 다음 07:30 스캔에서 오탐 없는지 확인.
