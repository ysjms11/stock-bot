...
0. **맥미니 git pull + 재시작** — 이번 세션 큰 변경 4건 배포 반영:
   - US 애널 3단계 완료 (75e0498/56a4bcc/47cb16a)
   - 주간 S&P 500 + Russell 1000 유니버스 (12cf948/975ef5d) — **다음 일요일 03:00 KST 03:00 수집 시작**
   - 관세청 수출 API 모듈 (feat 브랜치 f4625c9/604d775 + Agent B2 완료 대기)
   - INVESTMENT_RULES 레짐 개정 (6e2c6f9)

1. **Phase 2b 완료 후 feat/russell1000-universe → main 머지** — trade_api + MCP + 자동알림 일괄. 3-way merge (FF 불가, main도 앞서감).

2. **HS 매핑 세부 조정** — `data/hs_ticker_map.json` 검증에서 발견:
   - **Itemtrade API는 HS 6단위 이상 요구** (4단위 요청 시 $0 반환).
   - 수정 필요: LS ELECTRIC(8504→850422/23), HD조선(8901→890120), 두산에너빌리티/일진파워(8401→840110/20/40), 한미약품(3004→300410/20/90), 파마리서치, 에스티팜(2934→293499), 현대로템, 한화에어로.
   - Phase 3 test-writer가 라이브 검증하며 수정.

3. **5월 1일/11일/21일 수출 잠정치 첫 발송 확인** — 매월 3회 09:00 KST (Agent B2 구현 후). korea.kr PDF 파싱 정상 + 워치/포트 매핑 알림 정확성.

4. **다음 일요일 03:00 KST 주간 US harvest 첫 실행 확인** — 1,010종목 × ~33분 스캔. 애널 풀 확장 트래킹.

5. **DART 증분 수집 Phase6 모니터링** — 매일 02:00 KST. 분기 피크일(5/15, 8/14, 11/14) 신규 ~800종목 예상.

6. **4/17/4/8~4/11 공백 정책** — 미래만 정상화 확정. collect_daily backfill 불가 (현재가 API만 사용). 예외 케이스만 `backfill_gaps.py` 로 처리.

7. **KR_DEEPSEARCH 실전 검증** — 다음 한국 종목 딥서치 시 10 Step 누락 여부 / 킬 조건 체감 확인.
...
| 2026-04-18 | Oracle VM Stop 유지 확정 | 3일 무알림 확인, Terminate 불필요 (Free Tier 또는 디스크 소액). 실수 재시작 방지 필요 시 Resource Lock 추가 |
...
| 2026-04-19 | 거버넌스/밸류업 전체 롤백 | 3단계(자사주/배당/밸류업계획) 구현 후 사용자 판단으로 전체 제거. 후행지표(결과보고서) 비중 큼. 진짜 알파(배당 증액 비교, KRX 지수 편입 스크래핑)는 미구현. "간판만 비슷한 도구"라 알파 엣지 없음으로 판단. 재도전 시 각 단계별 **선행 시그널** 재설계 필요. 배포된 dd1c77c/0a820b8/e8598f8 3개 커밋 코드/스케줄/상수 모두 제거. |
| 2026-04-19 | `.claude/rules/schedule.md` 신규 | 반복잡 4종 + 일일잡 26종 전체 타임라인 표. 시간대 그룹(새벽/오전/오후/저녁 피크) + 저녁 피크 의존성 다이어그램 + 신규 잡 추가 절차. bot_architecture.md는 시스템 안정화(US 데이터 수집 등) 전까지 보류. stale 위험 < 실효성 판단. |
| 2026-04-21 | US 레이팅 오탐 근본 수정 (d1b2c1d) | `fetched_at` 기준 필터 → `rating_date >= date('now', '-4d')`. 첫 수집 시 수개월치 이력 전부 오탐하는 근본 버그. Hold→Hold 무변화 제외 + report_date + 경과일수 알림 추가. |
...
| 2026-04-23 | 관세청 수출 데이터 모듈 (f4625c9/604d775 + B2 완료 대기) | data.go.kr `Itemtrade`/`nitemtrade`/`nationtrade` 3 엔드포인트. 30종목 × 75 HS 매핑(`hs_ticker_map.json`). MCP `get_export_trend` 4 modes(summary/timeseries/country_breakdown/asp). 월 3회 자동 알림 + korea.kr PDF 잠정치. 유튜버 TRASS 스크래핑 대비 2일→안정. KOSPI-수출액 상관 0.93. 알파 검증 3개월 관찰 모드. |
...
| 2026-04-23 | HS 매핑 실증 기반 재조정 v2 | 병렬 딥서치(Agent 1 수치실증 + Agent 2 시장통찰)로 기존 주장 대부분 허위 확인. 라면 r=0.98 주장 → 실측 +0.41, 미용의료기기 r=0.97 → -0.57. 반도체는 동행(lag=0, r=+0.93~0.96). **고신뢰 8→3개 급감** (삼성전자/SK하이닉스/삼성전기만). 변압기 3종 confidence 0.8→0.35 (통관=후행, 수주=선행). 파마리서치 0.8→0.45 (330499 범용 HS). Phase 2에서 16개 후보(K-POP음반/산일전기/리노공업 등) 실증 검증 후 추가 예정. |
