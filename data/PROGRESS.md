...
0e. **US 애널 레이팅 2단계 실측 검증 (4/21~)** — `hourly_us_holdings_check` ET 12:00 / 16:30 평일 스케줄 + `daily_us_rating_scan` KST 07:30 (4/18, 24a9916). 첫 주말 지난 후 월요일 장중 실측 시작. 보유종목 실시간 감시 + 일일 요약 동작 확인. 결과 보고 나서 **3단계 스코프 재결정** (톱 100 애널 리스트/커버 종목 DB/`get_us_scan(discovery)`/주간 리포트/`/overview` 통합).
