...
### B. 비중 3단계 (단일 25% 하드캡 + 클러스터 35% 소프트캡 — INVESTMENT_RULES §3 정합 2026-06-04)
...
| **단일 하드캡** | **25%** | 절대 한도(시총 증가 포함). 초과 시 비중관리 축소 — 가격 익절 아님, 자금은 경로3 재배치 |
| **클러스터(섹터)** | **35% 소프트** | 의도적 매크로 베팅은 사전 기록 시 예외, 우발적 집중만 차단 |

- **단일 25%는 하드캡이다 (INVESTMENT_RULES §3 경로외).** 진입·보유 공통 절대 한도. 25% 이내에서는 우꼬리가 일하도록 끝까지 보유(Bessembinder, 가격 상승 ≠ 매도 — US_EXIT 절대원칙 0). 단 **이미 산 게 올라서 25% 초과 시에는 비중관리로 축소**하되, 이는 가격 익절(절대원칙 0 위반)이 아니라 영구손실 집중 리스크 관리이며 그 자금은 경로3 재배치로 돌린다.
- **큰 비중일수록 좌꼬리 게이트 엄격**: 영구손실 1건 충격이 비중에 비례(25% 종목 부도 시 포트 -25%, 회복 +33%/3~4년). 따라서 비중 20%+ 진입 시 Stage 1·2를 특히 깐깐하게.
...
### D. 매도 3경로 (US_EXIT·INVESTMENT_RULES §3 동기화 — 전량 이진, 가격≠매도)

> 상세는 `US_EXIT.md` / `INVESTMENT_RULES.md §3`. 요약:
> **대원칙: 가격 상승은 매도 사유 아님(재산정 트리거). 전량 보유/전량 매도 이진. thesis 붕괴로만 매도.**

**경로 1 — Thesis Invalidation (전량 청산) ★최우선**
- Stage 5 Kill Switch 발동 → **즉시 전량 청산**.
- 리더십 상실(연속 2분기 EPS 미스 + 가이던스 하향) → 3거래일 쿨링 후 **thesis 재평가 → 전량/보유 이진** (부분 50% 폐기). 대형주 PEAD 소멸(Martineau 2022, 2거래일 내 판단) / 소형주 <$2B 60일 드리프트.
- **공통 동인(AI capex 등) 2분기 연속 둔화 → 동인 묶인 종목 전체 재평가.**

**경로 2 — Technical Exit (스윙·모멘텀 한정)**
- 스윙만 -7~10% 하드 스톱(O'Neil/Minervini). 메인·가치는 기술 단독 매도 금지 → 경로1 재점검 알람. 재앙적 갭다운(-15%+/세션)은 매도 아닌 재평가 트리거.

**경로 3 — Opportunity Cost / 능동 재배치 (유일한 부분 비중 예외)**
- 검증 신규후보(3-Gate 3/3)가 있는데 풀투자면 능동 재배치 스캔. 양쪽 PDF 분해 forward-RR 비교, 재원=최저RR/최약thesis, 20거래일 빈도제한, 세제게이트. **INVESTMENT_RULES §3 능동 발동 기준이 단일 기준.**

- **밸류 sanity (재산정, 매도 아님)**: Implied perpetual g ≥ 10Y T-yield (Damodaran cap 위반) → **재산정 트리거**. 재산정 후 thesis 약화 시 경로3 검토.
- **포지션 구조 (경로외)**: 단일 25% 하드캡 초과 시 비중관리 축소(경로3 재배치). 클러스터(섹터) 35% 소프트캡(의도적 매크로 베팅 사전기록 예외).

### E. Core 3거래일 쿨링 (경로1 thesis 손상에만)
...
- **부분 정리 매도 30~50%** — Shiryaev 2008 bang-bang → 전량 이진 (경로3 재배치만 부분 예외, US_EXIT)
...
매도 3경로 (US_EXIT·INVESTMENT_RULES §3 동기화):
경로1. Thesis 훼손(Kill Switch) → 즉시 전량 / 리더십 상실(2분기 미스+가이드 하향) → 전량·보유 이진 (대형주 2거래일·소형주 60일) / 공통동인 2분기 둔화 → 동인 종목 전체 재평가
경로2. (스윙 한정) -7~10% 하드스톱 / 메인·가치는 기술 단독매도 금지(경로1 알람)
경로3. 능동 재배치 — 검증 신규 3/3 + 풀투자 시 PDF분해 forward-RR 비교(INVESTMENT_RULES §3 기준)
※ 밸류 sanity(Implied g ≥ 10Y) = 재산정 트리거(매도 아님) / 단일 25% 하드캡 초과는 비중관리 축소(경로3 재배치)
...
- **arbitrary (명시)**: Gate③ "2개"라는 정수, 단일 25% 하드캡·클러스터 35% 소프트캡 숫자 자체
