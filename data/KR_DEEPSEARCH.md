...
3. **가격 상승 ≠ 매도 사유** — 목표가 도달은 재산정(매도 아님). Bessembinder(2018) 우꼬리 절단 회피. 매도는 3경로 발동 시만 — thesis 붕괴(Kill Switch)·기술신호는 알람만·경로3 기회비용 재배치. 전량 이진(경로3 재배치만 부분 허용).
...
<position_sizing>
  3-Gate 통과: __/3 · 레짐 승수 🟢×1.0 / 🟡×0.8 / 🔴×0.5 (신규 진입 사이징 한정 — 🔴=발사 현금 다이얼(INVESTMENT_RULES §5)과 별개: 발사는 기보유 A급 추가, 승수는 신규 진입 축소)
  ...
  □ 단일 ≤25% 하드 □ 클러스터(섹터) 35% 소프트 □ 현금 최소선 (INVESTMENT_RULES §3 캡·§5 현금 정합)
</position_sizing>
...
> ✅ **도구 표기 전수 검증 완료 (2026-06-08 감사)**: 본 문서 표기 모두 현행 MCP 스키마와 일치.
