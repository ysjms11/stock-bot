...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 🔔 자동알림: 수출 데이터 월 3회 (관세청/산업통상자원부)
#   - trade_decade_report:  11일/21일 09:00 KST — korea.kr 10일/20일 잠정치 PDF 파싱
#   - trade_monthly_report: 매월 16일 03:00 KST — data.go.kr 전월 확정치 + 30종목 매핑
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
TRADE_SENT_FILE      = f"{_DATA_DIR}/trade_sent.json"
HS_TICKER_MAP_FILE   = f"{_DATA_DIR}/hs_ticker_map.json"
_KOREA_KR_BASE       = "https://www.korea.kr"
_TRADE_SEARCH_URL    = _KOREA_KR_BASE + "/briefing/pressReleaseList.do"
_TRADE_DETAIL_URL    = _KOREA_KR_BASE + "/briefing/pressReleaseView.do"
_TRADE_PDF_DIR       = f"{_DATA_DIR}/trade_pdfs"


def _yoy_icon(yoy: float | None) -> str:
    """YoY% → 아이콘 (≥30 ⚡ / 15~30 🟢 / -15~15 ➖ / -30~-15 🟡 / <-30 🔴)."""
    if yoy is None:
        return "❓"
    if yoy >= 30:
        return "⚡"
...
def _parse_trade_pdf(pdf_path: str) -> dict:
    """korea.kr 관세청/산업통상부 수출입 현황 PDF 파싱.

    반환: {exp_usd, imp_usd, exp_yoy_pct, imp_yoy_pct, period_label}
    - exp_usd / imp_usd : 백만달러 단위 (PDF 표 원값)
    - *_yoy_pct         : 전년동기대비 % (감소는 음수)
    - period_label      : "2026년 4월 1일 ~ 4월 10일" 또는 "2026년 3월 월간"
    실패 시 value = None, 크래시 금지.
    """
    import pdfplumber

    out: dict = {
        "exp_usd": None, "imp_usd": None,
        "exp_yoy_pct": None, "imp_yoy_pct": None,
        "period_label": None,
    }
    try:
        ...
        print(f"[trade_pdf] 파싱 실패 {pdf_path}: {e}")
        return out
    ...
    # period_label: "2026년 4월 1일 ~ 4월 10일" 또는 "2026년 3월 월간"
    m = re.search(
        r"(\d{4}년\s*\d{1,2}월(?:\s*\d{1,2}일\s*~\s*\d{1,2}월\s*\d{1,2}일|\s*월간))",
        text_flat,
    )
    if m:
        out["period_label"] = re.sub(r"\s+", " ", m.group(1)).strip()

    # YoY: summary 문장에서 추출 (2가지 포맷 대응)
    #   A) "수출은 252억 달러로 전년동기대비 36.7% 증가"
    #   B) "수출은 49.2% 증가한 866억 달러"
    def _extract_yoy(label: str) -> float | None:
        m = re.search(
            rf"{label}[은]?\s*[\d,]+억\s*달러[^.]{{0,80}}?(△?-?[\d.]+)\s*%\s*(증가|감소)",
            text_flat,
        )
        if not m:
            m = re.search(
                rf"{label}[은]?\s*(△?-?[\d.]+)\s*%\s*(증가|감소)한\s*[\d,]+억\s*달러",
                text_flat,
            )
        if not m:
            return None
        yoy = _num(m.group(1))
        if m.group(2) == "감소" and yoy is not None and yoy > 0:
            yoy = -yoy
        return yoy

    out["exp_yoy_pct"] = _extract_yoy("수출")
    out["imp_yoy_pct"] = _extract_yoy("수입")

    # 금액: 표 "당 월" 컬럼 (2025당월/2025누계/2026전월/2026당월/2026누계) → 4번째 값
    m = re.search(r"수\s*출\s+([\d,]+)\s+([\d,]+)\s+([\d,]+)\s+([\d,]+)\s+([\d,]+)", text)
    if m:
        out["exp_usd"] = _num(m.group(4))
    m = re.search(r"수\s*입\s+([\d,]+)\s+([\d,]+)\s+([\d,]+)\s+([\d,]+)\s+([\d,]+)", text)
    if m:
        out["imp_usd"] = _num(m.group(4))

    # fallback: summary "XXX억 달러" → 백만달러 (×100)
    if out["exp_usd"] is None:
        m = re.search(r"수출[은]?[^억]{0,50}?([\d,]+)억\s*달러", text_flat)
        if m:
            v = _num(m.group(1))
            if v is not None:
                out["exp_usd"] = v * 100.0
    if out["imp_usd"] is None:
        m = re.search(r"수입[은]?[^억]{0,50}?([\d,]+)억\s*달러", text_flat)
        if m:
            v = _num(m.group(1))
            if v is not None:
                out["imp_usd"] = v * 100.0
...
async def _trade_search_latest(session, keyword: str, days_back: int = 7) -> list[dict]:
    """korea.kr 보도자료 검색 → 최근 days_back일 이내 결과 리스트.

    반환: [{news_id, title, date(YYYY-MM-DD), agency}]
    실패 시 빈 리스트 (크래시 금지).
    """
    try:
        params = {"srchWord": keyword, "page": "1"}
        async with session.get(_TRADE_SEARCH_URL, params=params, timeout=20) as r:
            html = await r.text()
    except Exception as e:
        print(f"[trade_search] HTTP 실패: {e}")
        return []
    ...
    try:
        ...
        print(f"[trade_search] HTML 파싱 실패: {e}")
        return []

    out: list[dict] = []
    threshold = (datetime.now(KST).date() - timedelta(days=days_back))
    seen_ids: set = set()
    for a in soup.select('a[href*="pressReleaseView"]'):
        href = a.get("href", "")
        ...
        for _ in range(5):
            ...
            if container is None:
                break
            tx = container.get_text(separator="|", strip=True)
            dm = re.search(r"(\d{4}-\d{2}-\d{2})", tx)
            if dm:
                date_str = dm.group(1)
                # 기관은 보통 날짜 다음 토큰
...
async def _trade_fetch_pdf_url(session, news_id: str) -> tuple[str | None, str | None]:
    """korea.kr 상세 페이지에서 PDF 첨부 다운로드 URL + 파일명 추출.

    반환: (full_pdf_url, filename) 또는 (None, None).
    """
    try:
        async with session.get(
            _TRADE_DETAIL_URL, params={"newsId": news_id}, timeout=20
        ) as r:
            html = await r.text()
    except Exception as e:
        print(f"[trade_detail] HTTP 실패 {news_id}: {e}")
        return None, None
    ...
    # 각 download.do 링크의 부모 컨텍스트에서 파일명(.pdf) 찾기
    for a in soup.select('a[href*="download.do"]'):
        href = a.get("href", "")
        if "fileId=" not in href:
            continue
        parent_text = a.parent.get_text(separator=" ", strip=True) if a.parent else ""
        if ".pdf" not in parent_text.lower():
            continue
        # filename 추출
        fm = re.search(r"(\S+\.pdf)", parent_text)
        filename = fm.group(1) if fm else f"{news_id}.pdf"
        full_url = _KOREA_KR_BASE + href.replace("&amp;", "&")
        return full_url, filename
    return None, None


async def _trade_download_pdf(session, url: str, save_path: str) -> bool:
    """PDF 다운로드 → save_path 저장. 성공 True."""
    try:
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        async with session.get(url, timeout=60) as r:
            if r.status != 200:
                print(f"[trade_dl] HTTP {r.status}: {url}")
                return False
            data = await r.read()
        ...
        print(f"[trade_dl] 실패 {url}: {e}")
        return False


def _load_hs_ticker_map() -> dict:
    """hs_ticker_map.json 로드. 실패 시 빈 dict."""
    try:
        with open(HS_TICKER_MAP_FILE, encoding="utf-8") as f:
            data = json.load(f)
        # _meta 키 제거
        return {k: v for k, v in data.items() if not k.startswith("_") and isinstance(v, dict)}
    except Exception as e:
        print(f"[hs_map] 로드 실패: {e}")
        return {}


def _get_dominant_hs(entry: dict) -> dict | None:
    """hs_codes 리스트에서 dominant=True 또는 weight 최대값 hs 엔트리 반환."""
    codes = entry.get("hs_codes") or []
    if not codes:
        return None
    for c in codes:
        if c.get("dominant"):
            return c
    return max(codes, key=lambda c: c.get("weight", 0))


def _save_trade_preliminary_decade(
    period: str, parsed: dict, source_url: str
) -> bool:
    """trade_preliminary_decade 테이블 UPSERT. 성공 True."""
    try:
        from db_collector import _get_db
        conn = _get_db()
    except Exception as e:
        print(f"[trade_db] _get_db 실패: {e}")
        return False
    try:
        conn.execute(
            """INSERT OR REPLACE INTO trade_preliminary_decade
               (period, period_label, exp_usd, imp_usd, exp_yoy_pct, imp_yoy_pct,
            ...
            (
                ...
                parsed.get("exp_usd"),
                parsed.get("imp_usd"),
                parsed.get("exp_yoy_pct"),
                parsed.get("imp_yoy_pct"),
                source_url,
...
async def _compute_hs_ticker_yoy(
    tickers_entries: list[tuple[str, dict]], months_back: int = 12
) -> list[dict]:
    """각 종목의 dominant HS 코드 최근 월 YoY 계산.

    tickers_entries: [(ticker, hs_map_entry), ...]
    반환: [{ticker, name, hs_label, hs_code, confidence, current_usd, yoy_pct,
            icon, note(해외생산/수주 플래그 등)}]

    data.go.kr 제약: 조회기간은 1년 이내 (12개월) 만 허용. YoY 계산 위해
    end_yymm 과 13개월 전을 동시 포함하려면 두 번 호출 필요.
    """
    from trade_api import trade_item, compute_yoy_mom_qoq

    results: list[dict] = []
    now = datetime.now(KST)
    # 최근 12개월 (API 제약: max 1 year)
    end_yymm = now.strftime("%Y%m")
    y = now.year; m = now.month - (months_back - 1)
    while m <= 0:
        y -= 1; m += 12
    strt_yymm = f"{y:04d}{m:02d}"

    # YoY 계산용: 전년도 최근 N개월 (API 1년 제약 우회)
    # 최신 데이터가 1~2달 지연되므로 넉넉히 전년 3개월 커버
    prev_end_y = now.year - 1
    prev_end_m = now.month
    prev_start_y = prev_end_y
    prev_start_m = prev_end_m - 2  # 3개월 구간
    while prev_start_m <= 0:
        prev_start_y -= 1
        prev_start_m += 12
    prev_strt = f"{prev_start_y:04d}{prev_start_m:02d}"
    prev_end = f"{prev_end_y:04d}{prev_end_m:02d}"

    loop = asyncio.get_running_loop()

    for ticker, entry in tickers_entries:
        name = entry.get("name", ticker)
        conf = float(entry.get("confidence", 0) or 0)
        hs = _get_dominant_hs(entry)
        if not hs:
            continue
        hs_code = hs.get("code", "")
        ...
        try:
            rows = await loop.run_in_executor(
                None,
                lambda c=hs_code: trade_item(c, strt_yymm, end_yymm, 200),
            )
        except Exception as e:
            print(f"[trade_yoy] {ticker}/{hs_code} 실패: {e}")
            rows = []
        await asyncio.sleep(1.0)

        # 전년 동월 단독 조회 (1년 제약 우회)
        try:
            prev_rows = await loop.run_in_executor(
                None,
                lambda c=hs_code: trade_item(c, prev_strt, prev_end, 200),
            )
        except Exception as e:
            print(f"[trade_yoy] {ticker}/{hs_code} prev 실패: {e}")
            prev_rows = []
        await asyncio.sleep(1.0)

        merged = (rows or []) + (prev_rows or [])
        if not merged:
            results.append({
                "ticker": ticker, "name": name, "hs_label": hs_label,
                "hs_code": hs_code, "confidence": conf,
                "current_usd": None, "yoy_pct": None, "icon": "❓",
            })
            continue

        stats = compute_yoy_mom_qoq(merged, value_key="exp_usd")
        current = stats.get("current")
        yoy = stats.get("yoy_pct")
        # latest yymm 계산
        yymms = sorted({r.get("yymm") for r in merged if r.get("yymm")})
        latest = yymms[-1] if yymms else None
        results.append({
            "ticker": ticker, "name": name, "hs_label": hs_label,
            "hs_code": hs_code, "confidence": conf,
            "current_usd": current, "yoy_pct": yoy, "icon": _yoy_icon(yoy),
            "latest_yymm": latest,
        })
...
def _fmt_usd_m(amt: float | None) -> str:
    """**백만달러** 단위 입력 → 가독성 표시. (예: PDF 파서 값)
    None → '—'. ≥1000 → ${X.X}B, 그 외 → ${X}M.
    """
    if amt is None:
        return "—"
    if amt >= 1000:
        return f"${amt/1000:.1f}B"
    return f"${amt:.0f}M"


def _fmt_usd_raw(amt: float | None) -> str:
    """**USD 원값** 입력 → 가독성 표시. (예: data.go.kr trade_api)
    ≥1e9 → ${X.X}B, ≥1e6 → ${X}M, ≥1e3 → ${X}K, 그 외 → ${X}.
    """
    if amt is None:
        return "—"
    if amt >= 1e9:
...
# backward compat alias (PDF 파서 값 표시용)
_fmt_usd = _fmt_usd_m


async def _trade_send(context, text: str, parse_mode: str | None = "Markdown") -> bool:
    """context 가 None 이면 stdout 에 출력 (수동 dry-run), 있으면 텔레그램 발송.
    성공/None 모두 True 반환. 실패 시 로그 후 False.
    """
    if context is None or not hasattr(context, "bot"):
        print("[trade_send] (dry-run) message:")
        print(text)
        return True
    try:
        await context.bot.send_message(
        ...
        print(f"[trade_send] 텔레그램 발송 실패: {e}")
        return False


async def trade_decade_report(context):
    """매월 11일 / 21일 09:00 KST — 관세청 10일/20일 잠정치 알림.

    로직:
      1. korea.kr 에서 "수출입 현황 [잠정치]" 최근 보도자료 검색
      2. 오늘 날짜 기준 (11일=10일 잠정치, 21일=20일 잠정치) 매칭
      3. PDF 다운로드 + 파싱 → 총 수출액/YoY
      4. trade_preliminary_decade UPSERT
      5. 고신뢰(conf>=0.7) 종목 5개 YoY 계산 → 메시지 동봉
      6. 텔레그램 전송 + trade_sent.json 중복 방지

    수동 호출: context=None 또는 dry-run 객체 허용. context=None 이면 메시지 미전송,
               dict 반환 용도 (스모크 테스트). 스케줄 호출 시 context.job 존재.
    """
    now = datetime.now(KST)
    day = now.day
    year = now.year
    ...
    # 스케줄 가드: 11일 또는 21일만 실행 (수동 호출은 날짜 무관 진행)
    # JobQueue는 매일 09:00 트리거 → 내부에서 day 체크
    is_scheduled = context is not None and getattr(context, "job", None) is not None
    if is_scheduled and day not in (11, 21):
        return

    # 11일 → 10일 잠정치(_1), 21일 → 20일 잠정치(_2)
    # 수동호출 (day ≠ 11/21) 시 가장 최근 유형 추정
    if day >= 21:
        decade = "_2"; cutoff_day = 20
    elif day >= 11:
        decade = "_1"; cutoff_day = 10
    elif day >= 1:
        # 월초 수동호출: 전월 20일 잠정치
        decade = "_2"; cutoff_day = 20
    else:
    ...
    if day >= 11:
        ...
        target_year, target_month = pm_y, pm_m
    period = f"{target_year:04d}{target_month:02d}{decade}"

    # 환경변수 가드 (DATAGO_API_KEY 없으면 종목 YoY 파트 skip, PDF는 진행)
    has_datago = bool(os.environ.get("DATAGO_API_KEY", "").strip())

    # 중복 방지
    sent = load_json(TRADE_SENT_FILE, {})
    if sent.get(period):
        print(f"[trade_decade] {period} 이미 발송됨, skip")
        return

    import aiohttp
    async with aiohttp.ClientSession() as session:
        # 1. 검색
        results = await _trade_search_latest(session, "수출입 현황", days_back=14)
        await asyncio.sleep(1.0)

        # 2. 매칭: 타이틀에 "N월 1일 ~ N월 (10일|20일)" + "[잠정치]"
        target_keyword = (
            f"{target_month}월 1일" if cutoff_day == 10 else f"{target_month}월 1일"
        )
        cutoff_keyword = f"{target_month}월 {cutoff_day}일"
        matched = None
        for r in results:
            title = r.get("title", "")
            if cutoff_keyword not in title:
                continue
            if "잠정치" not in title:
                continue
            if f"{target_year}년" not in title and f"{target_year%100:02d}년" not in title:
                continue
            matched = r
            break
        ...
        if not matched:
            msg = (
                f"⚠️ 수출 잠정치 PDF 미확인 "
                f"({target_year}년 {target_month}월 1~{cutoff_day}일)\n"
                f"검색 결과 {len(results)}건 중 매칭 실패. 다음 스케줄에서 재시도."
            )
            await _trade_send(context, msg, parse_mode=None)
            return

        news_id = matched["news_id"]
        source_url = f"{_TRADE_DETAIL_URL}?newsId={news_id}"

        # 3. PDF URL 조회
        pdf_url, filename = await _trade_fetch_pdf_url(session, news_id)
        await asyncio.sleep(1.0)
        if not pdf_url:
            msg = (
                f"⚠️ 수출 잠정치 PDF 미확인 (newsId={news_id})\n"
                f"첨부 링크 추출 실패. 다음 스케줄에서 재시도.\n"
                f"📎 {source_url}"
            )
            await _trade_send(context, msg, parse_mode=None)
            return
        ...
        # 4. 다운로드 + 파싱
        save_path = f"{_TRADE_PDF_DIR}/{period}_{filename}"
        ok = await _trade_download_pdf(session, pdf_url, save_path)
        if not ok:
            msg = f"⚠️ 수출 잠정치 PDF 다운로드 실패 ({period})\n📎 {source_url}"
            await _trade_send(context, msg, parse_mode=None)
            return

    # aiohttp session 밖에서 PDF 파싱
    parsed = _parse_trade_pdf(save_path)
    if not parsed.get("exp_usd") or parsed.get("exp_yoy_pct") is None:
        msg = (
            f"⚠️ 수출 잠정치 PDF 파싱 실패 ({period})\n"
            f"파일: {os.path.basename(save_path)}\n"
            f"exp_usd={parsed.get('exp_usd')} yoy={parsed.get('exp_yoy_pct')}\n"
            f"📎 {source_url}"
        )
        await _trade_send(context, msg, parse_mode=None)
        return
    ...
    # 5. DB UPSERT
    _save_trade_preliminary_decade(period, parsed, source_url)

    # 6. 고신뢰 종목 YoY 계산 (워치/포트 교집합 우선)
    hs_map = _load_hs_ticker_map()
    watch = {}
    try:
    ...
    try:
        ...
        port_kr = {k: v for k, v in port.items()
                   if k not in ("us_stocks", "cash_krw", "cash_usd")
                   and isinstance(v, dict)}
    except Exception:
        port_kr = {}

    # conf ≥ 0.7 + (워치 ∪ 포트) 교집합. 없으면 전체 conf ≥ 0.7 중 상위 5개
    candidate_tickers = set(watch.keys()) | set(port_kr.keys())
    high_conf: list[tuple[str, dict]] = []
    for t, entry in hs_map.items():
        conf = float(entry.get("confidence", 0) or 0)
        if conf >= 0.7:
            high_conf.append((t, entry))
    # 교집합 우선
    intersect = [(t, e) for t, e in high_conf if t in candidate_tickers]
    if intersect:
        pick = intersect[:5]
    else:
        pick = sorted(high_conf, key=lambda x: -float(x[1].get("confidence", 0)))[:5]

    ticker_yoys: list[dict] = []
    if has_datago and pick:
        try:
            ticker_yoys = await _compute_hs_ticker_yoy(pick, months_back=12)
        except Exception as e:
            print(f"[trade_decade] HS YoY 계산 실패: {e}")

    # 7. 메시지 구성
    exp_usd_mil = parsed.get("exp_usd") or 0
    exp_b = exp_usd_mil / 1000.0  # 백만 → 십억달러
    exp_yoy = parsed.get("exp_yoy_pct")
    period_label = parsed.get("period_label", f"{target_year}년 {target_month}월 1~{cutoff_day}일")

    lines = [
        f"📊 *{period_label} 수출 잠정치*",
        f"전국 총 수출: *${exp_b:.1f}B* (YoY {exp_yoy:+.1f}%)"
        if exp_yoy is not None else
        f"전국 총 수출: *${exp_b:.1f}B*",
    ]
    imp_usd_mil = parsed.get("imp_usd")
    imp_yoy = parsed.get("imp_yoy_pct")
    if imp_usd_mil:
        imp_b = imp_usd_mil / 1000.0
        if imp_yoy is not None:
            lines.append(f"전국 총 수입: ${imp_b:.1f}B (YoY {imp_yoy:+.1f}%)")
        else:
            lines.append(f"전국 총 수입: ${imp_b:.1f}B")
    lines.append("")

    if ticker_yoys:
        lines.append("🟢 *고신뢰 종목 동향 (최신 월 기준)*:")
        latest_yymm = None
        for r in ticker_yoys:
            latest_yymm = latest_yymm or r.get("latest_yymm")
            cur = r.get("current_usd")
            yoy = r.get("yoy_pct")
            cur_str = _fmt_usd_raw(cur)  # data.go.kr raw USD
            yoy_str = f"YoY {yoy:+.1f}%" if yoy is not None else "YoY —"
            lines.append(
                f"• {r['name']} ({r['hs_label']}): {cur_str} {yoy_str} {r['icon']}"
            )
        if latest_yymm and len(latest_yymm) == 6:
            y, m = latest_yymm[:4], latest_yymm[4:]
            lines.append("")
            lines.append(
                f"⚠️ 참고: 이번 주 반영된 최신월은 {y}.{m}. "
                f"확정치는 매월 15일 업데이트."
            )
    elif pick and not has_datago:
        lines.append("ℹ️ 종목별 YoY: DATAGO_API_KEY 미설정으로 skip")
    lines.append("")
    lines.append(f"📎 원본: {source_url}")

    msg = "\n".join(lines)
    sent_ok = await _trade_send(context, msg, parse_mode="Markdown")
    if sent_ok and context is not None:
        sent[period] = datetime.now(KST).isoformat()
        save_json(TRADE_SENT_FILE, sent)
        print(f"[trade_decade] 발송 완료 ({period})")


async def trade_monthly_report(context):
    """매월 16일 03:00 KST — 전월 확정치 data.go.kr 풀수집 + 30종목 요약.

    로직:
      1. 전월 dominant HS 코드 30종목 전체 trade_item 호출 → DB 저장
      2. 고신뢰(conf≥0.7) TOP + 중신뢰(conf 0.4-0.69) 요약 + 피크아웃 경고
      3. 텔레그램 전송 + trade_sent.json 중복 방지
    """
    # 환경변수 가드
    if not os.environ.get("DATAGO_API_KEY", "").strip():
        print("[trade_monthly] DATAGO_API_KEY 미설정, skip")
        return

    now = datetime.now(KST)
    # 스케줄 가드: 16일만 실행 (수동호출 context=None 허용)
    is_scheduled = context is not None and getattr(context, "job", None) is not None
    if is_scheduled and now.day != 16:
        return

    # 전월 계산
    y = now.year if now.month > 1 else now.year - 1
    m = now.month - 1 if now.month > 1 else 12
    period = f"{y:04d}{m:02d}_M"

    sent = load_json(TRADE_SENT_FILE, {})
    if sent.get(period):
        print(f"[trade_monthly] {period} 이미 발송됨, skip")
        return

    hs_map = _load_hs_ticker_map()
    if not hs_map:
        print("[trade_monthly] hs_ticker_map 로드 실패")
        return

    from trade_api import trade_item, save_trade_monthly, compute_yoy_mom_qoq, compute_peakout_signal

    # 1. 최근 12개월치 시계열 수집 (data.go.kr 1년 제약)
    #    YoY 계산 위해 전년 동월 3개월 구간 추가 조회
    end_yymm = f"{y:04d}{m:02d}"
    sy = y; sm = m - 11
    while sm <= 0:
        sy -= 1; sm += 12
    strt_yymm = f"{sy:04d}{sm:02d}"

    # 전년 동월 ±3개월 (피크아웃 2차차분 + YoY 여유. API 1년 제약 우회)
    py_end_y = y - 1; py_end_m = m + 1  # +1 to extend
    if py_end_m > 12:
        py_end_y += 1; py_end_m -= 12
    py_strt_y = y - 1; py_strt_m = m - 3
    while py_strt_m <= 0:
        py_strt_y -= 1; py_strt_m += 12
    prev_strt_yymm = f"{py_strt_y:04d}{py_strt_m:02d}"
    prev_end_yymm = f"{py_end_y:04d}{py_end_m:02d}"

    loop = asyncio.get_running_loop()

    high_rows: list[dict] = []   # conf ≥ 0.7
    mid_rows: list[dict] = []    # 0.4 <= conf < 0.7
    low_rows: list[dict] = []    # conf < 0.4 (수주 우선 참고용)
    peakout_rows: list[dict] = []

    total_saved = 0

    for ticker, entry in hs_map.items():
        conf = float(entry.get("confidence", 0) or 0)
        name = entry.get("name", ticker)
        hs = _get_dominant_hs(entry)
        if not hs:
        ...
        try:
            rows = await loop.run_in_executor(
                None,
                lambda c=hs_code: trade_item(c, strt_yymm, end_yymm, 300),
            )
        except Exception as e:
            print(f"[trade_monthly] {ticker}/{hs_code} 수집 실패: {e}")
            rows = []
        await asyncio.sleep(1.0)

        # 전년 동월 구간 추가 조회 (YoY 계산)
        try:
            prev_rows = await loop.run_in_executor(
                None,
                lambda c=hs_code: trade_item(c, prev_strt_yymm, prev_end_yymm, 300),
            )
        except Exception as e:
            print(f"[trade_monthly] {ticker}/{hs_code} prev 실패: {e}")
            prev_rows = []
        await asyncio.sleep(1.0)

        # DB 저장은 당월/전월 모두
        all_rows = (rows or []) + (prev_rows or [])
        if all_rows:
            try:
                saved = await loop.run_in_executor(
                    None, lambda rs=all_rows: save_trade_monthly(rs, "datago")
                )
                total_saved += saved
            except Exception as e:
                print(f"[trade_monthly] save 실패 {ticker}: {e}")

        stats = compute_yoy_mom_qoq(all_rows, value_key="exp_usd") if all_rows else {}
        peak = compute_peakout_signal(all_rows, value_key="exp_usd") if all_rows else {}

        row_info = {
            "ticker": ticker, "name": name, "hs_label": hs_label,
            "confidence": conf, "current_usd": stats.get("current"),
            "yoy_pct": stats.get("yoy_pct"),
            "peakout": peak.get("peakout", False),
            "peak_reasons": peak.get("reasons", []),
        }
        if conf >= 0.7:
            high_rows.append(row_info)
        elif conf >= 0.4:
            mid_rows.append(row_info)
        else:
            low_rows.append(row_info)

        if peak.get("peakout") and stats.get("yoy_pct") is not None:
            peakout_rows.append(row_info)

    # 2. 메시지
    lines = [f"📈 *{y}년 {m}월 확정 수출 (종목별 매핑)*"]
    lines.append(f"DB 저장: {total_saved}행 · HS {len(hs_map)}종목")
    lines.append("")

    if high_rows:
        lines.append(f"🟢 *고신뢰 TOP (conf≥0.7, {len(high_rows)}종)*:")
        for r in sorted(high_rows, key=lambda x: -(x.get("yoy_pct") or -999)):
            cur = _fmt_usd_raw(r.get("current_usd"))  # data.go.kr raw USD
            yoy = r.get("yoy_pct")
            yoy_str = f"YoY {yoy:+.1f}%" if yoy is not None else "YoY —"
            icon = _yoy_icon(yoy)
            lines.append(f"• {r['name']}: {cur} {yoy_str} {icon}")
        lines.append("")

    if mid_rows:
        lines.append(f"🟡 *중신뢰 (conf 0.4-0.69, {len(mid_rows)}종)*:")
        parts = []
        for r in sorted(mid_rows, key=lambda x: -(x.get("yoy_pct") or -999)):
            yoy = r.get("yoy_pct")
            if yoy is None:
                parts.append(f"{r['name']} —")
            else:
                parts.append(f"{r['name']} {yoy:+.0f}%{_yoy_icon(yoy)}")
        # 2~3개씩 묶기
    ...
    if peakout_rows:
        lines.append(f"🔴 *피크아웃 경고 (2차차분<0 + MA 꺾임, {len(peakout_rows)}종)*:")
        for r in peakout_rows:
            yoy = r.get("yoy_pct")
            yoy_str = f"YoY {yoy:+.1f}%" if yoy is not None else "YoY —"
            reasons = "; ".join(r.get("peak_reasons", [])[:2])
            lines.append(f"• {r['name']}: {yoy_str} ({reasons})")
        lines.append("")

    if low_rows:
        lines.append(f"📌 *수주 우선 업종 (conf<0.4, {len(low_rows)}종, 참고용)*:")
        lines.append("  " + ", ".join(r["name"] for r in low_rows[:12]))
    ...
    if sent_ok and context is not None:
        sent[period] = datetime.now(KST).isoformat()
        save_json(TRADE_SENT_FILE, sent)
        print(f"[trade_monthly] 발송 완료 ({period}, DB {total_saved}행)")
...
def main():
    ...
    # 수출 데이터 월 3회 자동 알림 (Phase 2b): 관세청 잠정치 + 월간 확정치
    # 일 필터는 함수 내부에서 날짜 체크 (JobQueue는 day-of-month 기본 미지원 → 매일 09:00 실행하고 내부 guard)
    jq.run_daily(trade_decade_report,     time=dtime(9,  0, tzinfo=KST), name="trade_decade")
    jq.run_daily(trade_monthly_report,    time=dtime(3,  0, tzinfo=KST), name="trade_monthly")
